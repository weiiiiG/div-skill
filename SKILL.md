...
> - ⚠️ **组件库和框架：不得擅自替换或移除项目正在使用的 UI 组件库（Ant Design、Element UI、Bootstrap、Tailwind、Material UI、shadcn/ui 等）。重构只调整容器结构和文件组织，不改变组件的标签、类名、props 用法。**
...
| 借口 | 为什么是错的 |
|---|---|
| "nav 高度 56px 没问题" | 页面需适配不同字号/缩放；`height` 应改用 `min-height` |
| "header 的 height 是设计尺寸不是布局尺寸" | nav/header 高度决定整体布局结构，属于布局尺寸，必须用 `min-height` |
...
| "这个组件库不好用，顺便换成更好的" | 重构只改结构，换组件库应单独提交，混在一起无法排查问题 |
